...
function openSettings(){ if(document.querySelector(".setmodal"))return; if(typeof sfx==="function")sfx("click");
  ...
  ov.innerHTML=`<div class="setbox">
    ...
    <div class="setrow"><div class="setrowtop"><span class="setlbl">🎬 전투로그 연출</span><button type="button" class="settgl ${(typeof _bmPaced!=="undefined"&&_bmPaced)?'on':''}" data-fx="pace">${(typeof _bmPaced!=="undefined"&&_bmPaced)?"켜짐":"꺼짐"}</button></div><div class="setnote">전투 메시지가 한 줄씩 순차 등장(타격감↑) · 박스 탭하면 즉시 스킵 · 끄면 한 번에 표시</div></div>
  ...
  ov.querySelectorAll('.settgl[data-fx]').forEach(b=>{ b.onclick=()=>{ const f=b.dataset.fx; let on=false;   // 🖥 화면효과 토글들(흔들림·전투연출)
    if(f==="shake"){ on=(typeof FX!=="undefined")?!FX.shake:false; if(typeof fxShakeSetOn==="function")fxShakeSetOn(on); if(on&&typeof fxShake==="function")fxShake(); }
    else if(f==="pace"){ on=(typeof _bmPaced!=="undefined")?!_bmPaced:false; if(typeof combatPaceSetOn==="function")combatPaceSetOn(on); }
...
const PATCH_NOTES=[   // ⬆ 최신이 위. 새 패치 때 이 배열 맨 위에 추가만 하면 기록이 누적된다.
  ...
  { ver:"v2.31 · 8/30", title:"방어 개편 — 패링/회피 + 적 턴 무버튼", items:[
      ...
      "⚔️ <b>강공격 무조건 패링 삭제</b> — 이제 <b>확률</b>로 뜸(일반 공격·스킬에도). 강공격은 텔레그래프라 반응 확률이 조금 높지만 무조건은 아님",
      "🚫 <b>적 턴엔 '계속' 버튼 제거</b> — 알아서 진행(패링·회피 QTE 때만 버튼). 급하면 스페이스로 스킵",
    ]},
  { ver:"v2.30 · 8/30", title:"속공 확률 추가 하향", items:[
      "⚡ <b>속공 '진짜 간혹' 수준으로 재하향</b> — 민첩 비례는 유지(저 ~1.7% · 극민첩+집중 ~8%). 상한 14%→8%",
    ]},
  ...
  { ver:"v2.17 · 8/29", title:"다른 마을 · 보스 풀뷰 · 제작소 정리", items:[
      ...
      "🔨 <b>제작소 개편</b> — 세트 장비 / <b>일반 장비(드랍 티어·무기·방어구·악세)</b> / 내성 / 동료 룬을 <b>탭·칩</b>으로 분류. 이제 세트뿐 아니라 <b>일반 장비도 제작</b> 가능(파밍 대안)",
    ]},
  { ver:"v2.16 · 8/29", title:"무력화(그로기) 시스템 개편", items:[
      "💫 <b>무력화 스택 시스템</b> — '그로기 게이지'를 <b>무력화 게이지</b>로. <b>무력화 스킬</b>(💫 표시: 강타·무장파괴·처형·연속베기·용아파 등)로 게이지를 크게 쌓아 채우면 <b>그로기</b>(받는 피해 2배·행동 불가) 발동",
      "🏷 <b>무력화 스킬 표시</b> — 스킬 창·전투 메뉴에 💫무력화 뱃지, 누적 시 <b>💫무력화+N</b> 플로팅 — 어떤 스킬로 무너뜨릴지 고민이 생김",
    ]},
  ...
  { ver:"v2.9 · 8/29", title:"승리 화면 분리 · 타격감 · 밸런스", items:[
      ...
      "🏆 <b>승리 순간</b> — 처치하면 몬스터 <b>상태창은 그대로 남고</b> 스프라이트 자리만 상자로. <b>배틀로그는 그대로 유지</b>(⤢로 전체 열람), '전리품 확인'은 아래 버튼으로. 로그가 벽처럼 겹치던 문제도 해결",
      "📦 <b>상자가 혼자 사라지던 버그</b> 수정 — '전리품 확인'을 누를 때까지 상자는 그대로 유지(2.6초 자동소멸 제거)",
      "⚔️ <b>패링 타격감 강화</b> — 완벽 패링 시 화면 강진동+대형 히트+양방향 베기+묵직/치명 사운드, 양호 패링도 베기 이펙트",
      "🔮 <b>파훼 게이지 더 빡세게</b>(ATK×4.0→5.5) — 3턴 안에 거의 풀화력을 쏟아야 저지, 못 채우면 부분 저지로 위력만↓",
      "👹 <b>몬스터가 다 비슷하던 문제</b> — 높은 층에서 약한 잡몹 배제(층에 맞는 상위 티어만 등장)+층별 강화 곡선 상향(0.14→0.15 / 0.10→0.12 / 0.08→0.10)",
    ]},
  ...
  { ver:"v2.6 · 8/29", title:"전투 필드 다듬기", items:[
      ...
      "🖱 <b>드래그 시 텍스트 파랗게 선택되던 것 방지</b> · 메시지 <b>깜빡이는 ▼ 제거</b>(정적으로)",
      "🎨 메시지 확대 스크롤바 <b>테마색</b>(하얀 기본 제거) · 닫기 버튼 스크롤해도 항상 보이게 고정",
    ]},
...
function settingsScreen(){ clearLog(); setScene("⚙️","설정");
  ...
  acts.push({label:(typeof _bmPaced!=="undefined"&&_bmPaced)?"🎬 전투로그 연출 끄기":"🎬 전투로그 연출 켜기",act:()=>{ if(typeof combatPaceSetOn==="function")combatPaceSetOn(typeof _bmPaced!=="undefined"?!_bmPaced:false); settingsScreen(); }});
